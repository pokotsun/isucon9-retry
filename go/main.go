...
func getTransactions(w http.ResponseWriter, r *http.Request) {
	...
	for _, item := range items {
		...
		err = tx.Get(&ts, "SELECT t.id, t.status, t.item_id, s.reserve_id FROM `transaction_evidences` AS t RIGHT JOIN `shippings` AS s ON t.id = s.transaction_evidence_id WHERE s.item_id = ?", item.ID)
